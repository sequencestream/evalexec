...
//
// Every result must carry a non-empty eval_id, so generating one cannot live in
// the CLI alone: a library caller that omits it would otherwise receive a
// result that fails its own validation. Run fills it in for both paths.
...
func (g UUIDv7Generator) NewID() (string, error) {
	...
	if _, err := read(b[6:]); err != nil {
		return "", fmt.Errorf("evalexec: generate eval_id: %w", err)
	}
